...
def save_waveform_to_csv(values, options):
    ...
    volts, time = get_waveform_with_time(values, options)
...
def save_waveform_to_csv_multi_channel(values, options):
    """
...
def ossiloscope(values, options):
...
def ossiloscope_data_preprocessing(values, options):
    """
...
def handle_incoming_data(method, values, options):
    """
    ...
    if method in methods:
        return methods[method](values, options)
    else:
...
if __name__ == "__main__":
    ...
    options = {"padding_value": 0.5, "operation": "additive"}
    result = handle_incoming_data(method_name, binary_values, options)
    print(result)
